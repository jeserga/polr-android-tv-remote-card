...
export class HistoryCard extends LitElement {
  ...
  override disconnectedCallback(){super.disconnectedCallback();this.generation++;this.loaded=false;this.loading=false;this.result={events:[],segments:[],daily:[]};}
  ...
  protected override render(){
    ...
    return html`<ha-card><div class="body"><header><div><h2>Historial de la TV</h2><p>Privado · jeserga · Horas de Madrid</p></div><button aria-label="Actualizar historial" @click=${()=>void this.load()}><ha-icon icon="mdi:refresh"></ha-icon></button></header>
      ...
      <nav>${[["sessions","Sesiones"],["events","Acciones"],["daily","Resúmenes diarios"]].map(([v,l])=>html`<button aria-pressed=${this.mode===v} @click=${()=>this.mode=v}>${l}</button>`)}</nav>
