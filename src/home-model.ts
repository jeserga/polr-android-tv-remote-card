...
export const clockTime=(value:string|number):string=>new Date(typeof value==="number"?value*1000:value).toLocaleString("es-ES",{timeZone:"Europe/Madrid",day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"});
...
export function sessionsFromSegments(segments:any[],now=Date.now()/1000){
  ...
  return sessions.reverse().map(s=>({...s,active:now-s.end<20}));
}
